...
import * as schoolStyles from "./school.module.css"
...
export default function School({ data, title, subtitle, date, image, imgAlt, desc, link, courses, activities }) {
  ...
  */
  ...
  function coursesJSX(obj) {
    ...
    for (let i = 0; i < obj.topicList.length; i++) {
      ...
      rtn.push(<React.Fragment><dd>{topic_obj.name} ({topic_obj.links.map(link_obj => {
          return (<React.Fragment><a href={link_obj.link} target="_blank" rel="noreferrer">{link_obj.abbrev}</a></React.Fragment>)
  ...
  function activitiesJSX(obj) {
    ...
    for (let i = 0; i < obj.topicList.length; i++) {
      let topic_obj = obj.topicList[i]
      rtn.push(<React.Fragment> { topic_obj.link? <a href={topic_obj.link} target="_blank" rel="noreferrer">{topic_obj.name}</a> : topic_obj.name}</React.Fragment>)      
